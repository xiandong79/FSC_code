...
class Scheduler:
    ...
    def sort_tasks(self):
        if self.scheduler_type == "MTTC":
            self.task_buffer.sort(key=lambda x: sum(self.cluster.users[x.job.user_id].preference_value[index] * v for index, v in self.cluster.users[x.job.user_id].alloc.iteritems()) / sum(
                self.cluster.users[x.job.user_id].preference_value[u] * self.cluster.users[x.job.user_id].ownership[u] for u in range(len(self.cluster.users[x.job.user_id].ownership))))
                    ...
                    for machineId in self.cluster.make_offers(task.stage.job.user_id):
                        if self.cluster.users[task.stage.job.user_id].preference_value[machineId] != 0:
                            if self.cluster.users[task.stage.job.user_id].preference_value[machineId] >= task.threshold:
                                ...
                                task.runtime = int(task.runtime /
                                                   self.cluster.users[task.stage.job.user_id].preference_value[machineId])

...
class Scheduler:
    ...
    def sort_tasks(self):
        if self.scheduler_type == "isolated" or self.scheduler_type == "MTTC+DS":
        ...
        for stage in self.task_buffer:
            ...
            for task in tmpList:
                ...
                if self.scheduler_type == "isolated":
                    # above line is added by Xiandong
                        ...
                        if self.cluster.users[task.stage.job.user_id].preference_value[machineId] != 0:
                            if self.cluster.users[task.stage.job.user_id].alloc[machineId] < self.cluster.users[task.stage.job.user_id].ownership[machineId]:
                                ...
                                if machineId in self.stageIdToAllowedMachineId[task.stage.id]:
                                    ...
                                    break
                elif self.scheduler_type == "DelaySched":
                    # added for scheduler_type == "delay scheduling" by xiandong
                    for machineId in self.cluster.make_offers():
                        if self.cluster.users[task.stage.job.user_id].preference_value[machineId] != 0:
                            if self.cluster.users[task.stage.job.user_id].preference_value[machineId] >= task.threshold:
                                ...
                                if task.first_attempt_time > 0:
                                    ...
                                    if time - task.first_attempt_time > task.timeout:
                                        task.threshold -= task.threshold_step
                                        # print(
                                        #     "task.threshold is changing. And 'task.stage.job.id' is: ", task.stage.job.id, "the machine giving up is: ", machineId)
                                else:
                                    task.first_attempt_time = time
                else:  # scheduler_type = "MTTC+DS". DS = delay scheduling
                    for machineId in self.cluster.make_offers():
                        if self.cluster.users[task.stage.job.user_id].preference_value[machineId] != 0:
                            if self.cluster.users[task.stage.job.user_id].preference_value[machineId] >= task.threshold:
                                if self.cluster.users[task.stage.job.user_id].alloc[machineId] < self.cluster.users[task.stage.job.user_id].ownership[machineId]:
